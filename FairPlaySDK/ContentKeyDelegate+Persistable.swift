...
extension ContentKeyDelegate {
    ...
    func handlePersistableContentKeyRequest(keyRequest: AVPersistableContentKeyRequest) {
        ...
        guard let contentKeyIdentifierString = keyRequest.identifier as? String,
            let contentKeyIdentifierURL = URL(string: contentKeyIdentifierString) else {return}
...
//                let urlToPersistableKey = urlForPersistableContentKey(withContentKeyIdentifier: "amb-key")
                ...
                /*
                 ...
                 decrypting content.
                 */
                if let assetName = self.contentKeyToStreamNameMap.removeValue(forKey: assetIDString){
    ...
    func persistableContentKeyExistsOnDisk(withContentKeyIdentifier contentKeyIdentifier: String) -> Bool {
        let contentKeyURL = urlForPersistableContentKey(withContentKeyIdentifier: contentKeyIdentifier)
        print("key url on disk ----->\(contentKeyURL)----------------is it exist ---\(FileManager.default.fileExists(atPath: contentKeyURL.path))")
    ...
    func urlForPersistableContentKey(withContentKeyIdentifier contentKeyIdentifier: String) -> URL {
        let removeSKD = contentKeyIdentifier.replacingOccurrences(of: "skd://", with: "")
        var str = ""
        //= removeSKD.replacingOccurrences(of: "/", with: "")
        if removeSKD.contains(";"){
            let arr = removeSKD.components(separatedBy: ";")
            str = arr.last!
        }else {
            let arr = removeSKD.components(separatedBy: "/")
                str = arr.last!
        ...
        print("final path-------->\(str)")
        return contentKeyDirectory.appendingPathComponent("\(str)-Key")
        //return contentKeyDirectory.appendingPathComponent("amb-Key")
    ...
    func writePersistableContentKey(contentKey: Data, withContentKeyIdentifier contentKeyIdentifier: String) throws {
        ...
        do {
            print("writing on ----> in\n /n \(fileURL)")
            //fps.ezdrm.comapilicenses75b51abc-9bce-4b96-a552-805eea397733

        try contentKey.write(to: fileURL, options: Data.WritingOptions.atomicWrite)
        }catch{
            print("writing error ---->\(error) in\n /n \(fileURL)")
        }
    }
